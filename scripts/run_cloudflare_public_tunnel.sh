...
CLOUDFLARED_BIN="${BUNNY_CLOUDFLARE_TUNNEL_BIN:-$(command -v cloudflared || true)}"
TOKEN_FILE="${BUNNY_CLOUDFLARE_TUNNEL_TOKEN_FILE:-}"
...
TOKEN="${BUNNY_CLOUDFLARE_TUNNEL_TOKEN:-$(load_token_from_file)}"
...
if [ -z "$CLOUDFLARED_BIN" ] || [ ! -x "$CLOUDFLARED_BIN" ]; then
  ...
  echo "Install it or set BUNNY_CLOUDFLARE_TUNNEL_BIN first."
  exit 1
fi
...
if [ -z "$TOKEN" ]; then
  ...
  echo "  export BUNNY_CLOUDFLARE_TUNNEL_TOKEN_FILE=/path/to/cloudflare.token"
